...
export function ChatBox() {
  ...
  return (
    <div className="flex flex-col h-full min-h-[280px]">
      ...
      <div className="flex-1 overflow-auto rounded-xl border border-slate-200/60 p-4 space-y-3 bg-gradient-to-b from-slate-50/50 to-white/50 dark:from-slate-800/50 dark:to-slate-900/50 dark:border-slate-700/60">
        {messages.map((m, i) => (
          <div key={i} className={`flex ${m.isYou ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[80%] rounded-2xl px-4 py-2 text-sm ${
              ...
              {!m.isYou && <div className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">{m.sender}</div>}
              <div>{m.text}</div>
            </div>
          </div>
        ))}
