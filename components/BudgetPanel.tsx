  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="rounded-xl bg-gradient-to-br from-slate-100 to-slate-50 dark:from-slate-700 dark:to-slate-800 p-4">
          <div className="text-sm text-slate-500 dark:text-slate-400">Total spend</div>
          <div className="text-2xl font-bold tabular-nums text-slate-900 dark:text-slate-100">${t.toFixed(2)}</div>
        </div>
        <div className="rounded-xl bg-gradient-to-br from-emerald-50 to-green-50 dark:from-emerald-900/30 dark:to-green-900/30 p-4">
          <div className="text-sm text-emerald-600 dark:text-emerald-400">Per person</div>
          <div className="text-2xl font-bold tabular-nums text-emerald-700 dark:text-emerald-300">${perPerson}</div>
        </div>
      </div>
      
      <div className="space-y-2">
        <div className="text-sm font-medium text-slate-600 dark:text-slate-400">Breakdown</div>
        <ul className="rounded-xl border border-slate-200/60 divide-y divide-slate-100 bg-white/80 dark:border-slate-700/60 dark:divide-slate-700/60 dark:bg-slate-800/80">
          {shares.map((s, i) => (
            <li key={s.name} className="flex items-center justify-between px-4 py-3">
              ...
              <span className="tabular-nums font-semibold text-slate-700 dark:text-slate-300">${s.share}</span>
            </li>
          ))}
        </ul>
      </div>
      
      <div className="flex items-center justify-between p-4 rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200/60 dark:border-amber-800/40">
        <div className="flex items-center gap-2 text-sm text-amber-700 dark:text-amber-300">
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
          Jordan owes Alex $300
        </div>
        <button className="btn-secondary text-xs px-3 py-1.5">Settle up</button>
      </div>
    </div>
  )
}
